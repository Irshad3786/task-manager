...
function Dashboard() {
   ...
   const [Indexval,setIndexval] = useState()
    ...
    const Projecttittle = location.state
   ...
   const AllTaskDis =()=>{
    ...
    const submit=()=>{
      if(!Tittle || !Description || !Status){
        ...
        axios.post(`${import.meta.env.VITE_BACKEND_URL}/Task`,{Email,Tittle,Description,Status,Projecttittle})
        .then((res)=>{
    ...
    const AllTaskButton = ()=>{
      ...
      axios.post(`${import.meta.env.VITE_BACKEND_URL}/AllTasks`,{Email,Projecttittle},{ withCredentials: true })
      .then((res)=>{
    ...
    const DeleteTask =(data)=>{
      ...
      axios.post(`${import.meta.env.VITE_BACKEND_URL}/DeleteTask`,{_id: data._id,Email:Email,Projecttittle },{ withCredentials: true })
      .then((res)=>{
        AllTaskButton()
      })
      .catch((error)=>{
    ...
    const UpdateText = (data)=>{
      ...
      axios.post(`${import.meta.env.VITE_BACKEND_URL}/UpdateTask`,{_id: data._id , Description :Text , Email:Email,Projecttittle},{ withCredentials: true })
      .then((res)=>{
        console.log(res);
        AllTaskButton()
      })
      ...
      <div className='w-[100%] min-h-screen bg-[#FFF100] '>
          ...
          {!AllTaskBut && (<div className=''>
            ...
            <div className='flex flex-col items-center justify-center w-[100%] h-96 md:h-[75%] gap-4'>
              ...
              <div>
                <select  className='bg-[#006BFF] px-4 py-2 font-Afacad text-white' value={Status} onChange={(e)=>{setStatus(e.target.value)}}>
        ...
        {AllTaskBut && (<div className='relative'>
          ...
          {TaskData.slice().reverse().map((data,index)=>(
            ...
            <div className='absolute pt-2'>
              <h1 className='bg-black text-white px-2 font-Afacad rounded-s-2xl rounded-e-2xl'>{data.Status}</h1>
            </div>
          </div>
          ))}
